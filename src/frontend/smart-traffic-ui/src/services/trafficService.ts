...
class TrafficService {
  ...
  async getTrafficLight(id: string): Promise<TrafficLight> {
    const response = await ApiService.get<TrafficLight>(`/traffic/lights/${id}`);
    return response;
  }

  async updateTrafficLight(id: string, config: TrafficLightConfigForm): Promise<TrafficLight> {
    const response = await ApiService.put<TrafficLight>(`/traffic/lights/${id}`, config);
    return response;
  }

  async controlTrafficLight(id: string, phase: number, duration: number): Promise<void> {
    await ApiService.post(`/traffic/lights/${id}/control`, { phase, duration });
  }

  async getTrafficLightStatus(id: string): Promise<{ status: string; phase: number; duration: number }> {
    const response = await ApiService.get<{ status: string; phase: number; duration: number }>(`/traffic/lights/${id}/status`);
    return response;
  }
  ...
  async getVehicles(filters?: FilterOptions): Promise<Vehicle[]> {
    ...
    const response = await ApiService.get<Vehicle[]>(`/traffic/vehicles?${params.toString()}`);
    return response;
  }
  ...
  async getVehiclesByIntersection(intersectionId: string): Promise<Vehicle[]> {
    const response = await ApiService.get<Vehicle[]>(`/traffic/intersections/${intersectionId}/vehicles`);
    return response;
  }

  // Intersections
  async getIntersections(): Promise<Intersection[]> {
    const response = await ApiService.get<Intersection[]>('/traffic/intersections');
    return response;
  }

  async getIntersection(id: string): Promise<Intersection> {
    const response = await ApiService.get<Intersection>(`/traffic/intersections/${id}`);
    return response;
  }

  async getIntersectionMetrics(id: string, timeRange?: { start: Date; end: Date }): Promise<PerformanceMetrics[]> {
    const params = new URLSearchParams();
    if (timeRange) {
    ...
    const response = await ApiService.get<PerformanceMetrics[]>(`/traffic/intersections/${id}/metrics?${params.toString()}`);
    return response;
  }
  ...
  async getPerformanceMetrics(timeRange?: { start: Date; end: Date }): Promise<PerformanceMetrics[]> {
    ...
    const response = await ApiService.get<PerformanceMetrics[]>(`/traffic/metrics?${params.toString()}`);
    return response;
  }

  async getCurrentMetrics(): Promise<PerformanceMetrics> {
    const response = await ApiService.get<PerformanceMetrics>('/traffic/metrics/current');
    return response;
  }
  ...
  async getTrafficAnalytics(filters?: FilterOptions): Promise<{
    ...
    const response = await ApiService.get<typeof response>(`/traffic/analytics?${params.toString()}`);
    return response;
  }

  // Reports
  async generateReport(type: 'summary' | 'detailed' | 'export', filters?: FilterOptions): Promise<Blob> {
    const params = new URLSearchParams();
    ...
    const response = await ApiService.get(`/traffic/reports?${params.toString()}`, {
      responseType: 'blob'
    });
