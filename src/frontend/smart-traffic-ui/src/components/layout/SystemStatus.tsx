    ...
    <Panel className="system-status-panel">
      <PanelHeader>
        <div className="panel-header-content">
          <div className="header-left">
            ...
            <Badge colorScheme={overallStatus === 'healthy' ? '8' : overallStatus === 'degraded' ? '2' : '1'}>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="overall-status">
              <MessageStrip
                design={overallStatus === 'healthy' ? MessageStripDesign.Success : 
                       overallStatus === 'degraded' ? MessageStripDesign.Warning : 
              ...
              >
                <div className="status-content">
                  ...
                  <div className="status-text">
                    ...
                    <Text>{healthyServices} of {totalServices} services operational</Text>
                  </div>
                </div>
              </MessageStrip>
            </div>
            ...
            <div className="system-info">
              <div className="info-item">
                ...
                <div className="info-content">
                  <Text>Uptime</Text>
                  <Text className="info-value">{formatUptime(systemUptime)}</Text>
                </div>
              </div>
              <div className="info-item">
                <Icon name="refresh" />
                <div className="info-content">
                  ...
                  <Text className="info-value">{lastUpdate.toLocaleTimeString()}</Text>
                </div>
              </div>
              <div className="info-item">
                <Icon="connected" />
                <div className="info-content">
                  <Text>WebSocket</Text>
                  <Text className="info-value">{isConnected ? 'Connected' : 'Disconnected'}</Text>
                </div>
              </div>
            </div>
            ...
            <div className="config-section">
              ...
              <div className="config-items">
                <div className="config-item">
                  ...
                  <Badge colorScheme={systemConfig.simulation.enabled ? '8' : '9'}>
                    {systemConfig.simulation.enabled ? 'Enabled' : 'Disabled'}
                  </Badge>
                </div>
                <div className="config-item">
                  <Text>Data Export</Text>
                  <Badge colorScheme={systemConfig.dataExport.enabled ? '8' : '9'}>
                    {systemConfig.dataExport.enabled ? 'Enabled' : 'Disabled'}
                  </Badge>
                </div>
                <div className="config-item">
                  <Text>Notifications</Text>
                  <Badge colorScheme={systemConfig.notifications.enabled ? '8' : '9'}>
                    {systemConfig.notifications.enabled ? 'Enabled' : 'Disabled'}
                  </Badge>
                </div>
              </div>
            </div>
      ...
      <div className="system-status-footer">
        <Toolbar>
          <Button
            design="Transparent"
            onClick={() => {/* Navigate to system logs */}}
          >
            View System Logs
          </Button>
          <ToolbarSpacer />
